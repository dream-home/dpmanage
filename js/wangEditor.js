...
//  E.isOnWebsite = E.websiteHost.indexOf(E.hostname) >= 0;   设置不上传图片api
	E.isOnWebsite = true;
										...
										xhr.onreadystatechange=function(){
										    if (xhr.readyState==4){
										      ...
										      //document.getElementById("myDiv").innerHTML=data.result.domain+JSON.parse(xhr.responseText).hash;  
										   	 src = data.result.domain+JSON.parse(xhr.responseText).hash;
										   	  //img_area.src = data.result.domain+JSON.parse(xhr.responseText).hash;
										   	   var html = '<img src="' + src + '" style="max-width:70%;"/>';
        ...
        editor.xhrUploadImg = function (opt) {
            ...
            xhr.onload = function () {
                ...
                if (fileName.indexOf('.') > 0) {
                    ...
                    //console.log(editor.uploadImgOriginalName);
                    //alert(editor.uploadImgOriginalName.length)
                }
